...
## 12. Advanced Patterns: SubWorkflows & Ensemble Management

This section evaluates two advanced Pegasus capabilities referenced in [`pegasus-docs.txt`](pegasus-docs.txt) and their applicability to the FL workflow.

### 12.1 Pegasus `SubWorkflow` (Nested DAGs)

#### What it is
Pegasus provides a `SubWorkflow` class (part of `Pegasus.api.workflow`) that allows embedding one Pegasus workflow as a single job node inside a parent workflow. This creates a **hierarchical / workflow-of-workflows** DAG.

```python
from Pegasus.api import SubWorkflow, File

# Embed an unplanned sub-workflow (planned by parent planner)
round_subwf = SubWorkflow(
    f"round_{r}.yml",
    is_planned=False,
    _id=f"fl_round_{r}"
)
...
#### Applicability to FL Workflow — **HIGHLY APPLICABLE**

| Use Case | Benefit | Complexity |
|---|---|---|
| **Per-client data prep as SubWorkflow** | Encapsulate `ingest → preprocess → compute_stats` into a reusable, testable unit. Parent DAG shows one node per client instead of three. | Low |
| **FL Round as SubWorkflow** | Each round (`train_all → aggregate`) becomes a black-box node. Sequential rounds chain cleanly. Reduces main DAG from `O(R×C)` nodes to `O(R)` nodes. | Medium |
| **Evaluation as SubWorkflow** | Encapsulate `evaluate_tcia → evaluate_nih → merge_metrics` as a single evaluation stage. | Low |

#### Recommended SubWorkflow Architecture

```
Parent Workflow (fl-chest-workflow.yml)
├── [SubWorkflow] client_prep_LIDC.yml      ──┐
├── [SubWorkflow] client_prep_NSCLC.yml       │ Parallel
├── [SubWorkflow] client_prep_RIDER.yml     ──┘
├── initialize_model
├── FOR round = 1..R:
│   └── [SubWorkflow] fl_round_{r}.yml   (train_all + aggregate)
├── [SubWorkflow] final_eval.yml            (evaluate + report)
```

Each `fl_round_{r}.yml` is generated dynamically and contains:
```
round_{r}.yml
├── train_client_0  ──┐
├── train_client_1    │ Parallel fan-out
├── ...               │
├── train_client_N  ──┘
└── aggregate         (fan-in)
```

#### File Staging Between Parent and Child
- Global model passed from parent to child via `File()` object shared across `SubWorkflow` job inputs.
- Local models produced inside child and consumed by child's `aggregate` node — no parent visibility needed.
- Only the **aggregated global model** and **round metrics** are staged out of the child back to the parent.

#### When to Use
- **Recommended** when running ≥5 clients across ≥5 rounds. The flat DAG becomes unwieldy (>50 jobs). SubWorkflows improve readability, modularity, and independent testing of each round.
- **Required** if extending to hierarchical FL (e.g., hospital → regional → national aggregation levels).

### 12.2 Pegasus Service & Ensemble Manager

#### What it is
[Pegasus Service](https://pegasus.isi.edu/documentation/reference-guide/pegasus-service.html) exposes a REST API and web dashboard. The **Ensemble Manager** (`pegasus-em`) manages collections of workflows with throttling, priorities, and triggers.

#### Applicability to FL Workflow — **OPERATIONALLY APPLICABLE**

| Use Case | Benefit |
|---|---|
| **Hyperparameter / strategy sweep** | Submit multiple FL workflows (FedAvg, FedProx, SCAFFOLD) as an ensemble. Throttle `max_running=2` to avoid GPU starvation. |
| **Continuous / streaming FL** | Use `pegasus-em file-pattern-trigger` to auto-submit a new training round when new client data lands in a staging directory. |
| **Science gateway integration** | REST API allows external portals to launch FL experiments and poll status without direct submit-host access. |
| **Monitoring** | Web dashboard provides visual workflow status, useful for long-running FL training (6–12 hours). |

#### Example Ensemble Commands

```bash
# Create ensemble for FL experiments
pegasus-em create fl_experiments

# Submit FedAvg workflow (priority 10)
pegasus-em submit fl_experiments.fedavg ./plan.sh workflow_fedavg.yml -p 10

# Submit FedProx workflow (priority 5)
pegasus-em submit fl_experiments.fedprox ./plan.sh workflow_fedprox.yml -p 5

# Throttle to 2 concurrent GPU workflows
pegasus-em config fl_experiments -P 1 -R 2

# Auto-submit on new TCIA data arrival
pegasus-em file-pattern-trigger \
  fl_experiments \
  tcia_trigger \
  1h \
  ./workflow_generator.py \
  /staging/tcia/*/manifest.csv \
  --timeout 7d
```

#### When to Use
- **Not required** for a single FL experiment. Add when evolving to a production FL platform or running systematic ablation studies.
- **Recommended** for multi-tenant environments where multiple researchers share the same GPU cluster.

### 12.3 Summary Decision Matrix

| Capability | Current SPEC Pattern | When to Upgrade |
|---|---|---|
| Flat DAG (`Workflow`) | Default in Phases 1–7 | Always start here |
| `SubWorkflow` (nested) | Phases 8+ or if >5 rounds | Scaling, modularity, hierarchical FL |
| `Ensemble Manager` | Operational layer | Multi-experiment campaigns, continuous learning |
...
| Phase | Task | Files |
|---|---|---|
| **1** | Scaffold flat workflow structure | `workflow_generator.py`, `bin/`, `Docker/` |
...
| **7** | Full-scale flat DAG run with multiple clients | condorpool submission |
| **8** | Add NIH Chest X-Ray14 evaluation | `bin/ingest_nih.py`, cross-dataset eval |
| **9** | **(Optional)** Refactor to SubWorkflows | `bin/generate_round_workflow.py`, parent `workflow_generator.py` |
| **10** | **(Optional)** Add Ensemble Manager triggers | `pegasus-em` cron/file-pattern triggers for continuous FL |
